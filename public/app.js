...
$("builderCreate").onclick = async () => {
  ...
  $("builderOpen").disabled = true;
  $("builderCopy").disabled = true;
  $("builderRepo").disabled = true;

  try {
    const references = await filesToPayload($("builderFiles"));
    progress(1, 6, "Gemini is planning the website architecture");
    ...
    for (let i = 0; i < plan.files.length; i++) {
      ...
      progress(2, 6, `Generating file ${i + 1} of ${plan.files.length}: ${file.path}`, { manifest });
      const result = await call("/api/website-builder/generate-file", {
        method: "POST", headers: builderHeaders(),
        body: JSON.stringify({ title, slug, model, specification, files: references, manifest, path: file.path, purpose: file.purpose }),
      });
    ...
    const setup = await call("/api/website-builder/setup", {
      ...
      body: JSON.stringify({
        title, slug, private: $("builderPrivate").checked,
        needsD1: plan.needsD1, needsR2: plan.needsR2, migrationSql: plan.migrationSql,
      }),
    });
    ...
    const wrangler = {
      ...
      ...(setup.d1 ? { d1_databases: [{ binding: "DB", database_name: setup.d1.name, database_id: setup.d1.id }] } : {}),
      ...(setup.r2 ? { r2_buckets: [{ binding: "BUCKET", bucket_name: setup.r2.name }] } : {}),
    };
    ...
    for (let i = 0; i < entries.length; i++) {
      ...
      await call("/api/website-builder/publish-file", {
        method: "PUT", headers: ghHeaders(),
        body: JSON.stringify({ owner: setup.owner, repo: setup.repo, path, content }),
      });
    }
    ...
    const deployed = await call("/api/website-builder/deploy", {
      ...
      body: JSON.stringify({
        slug,
        workerCode: generated.get("src/index.js"),
        databaseId: setup.d1?.id || "",
        bucketName: setup.r2?.name || "",
      }),
    });

    progress(6, 6, "Website created successfully");
    const final = {
      status: 201, ok: true, public_url: deployed.public_url, repository: setup.repository,
      summary: plan.summary, files: entries.length, d1: setup.d1, r2: setup.r2,
    };
    show(final);
    $("builderStatus").textContent = `Website published: ${deployed.public_url}`;
    $("builderOpen").disabled = false;
    $("builderCopy").disabled = false;
    $("builderRepo").disabled = false;
...
$("factoryCreate").onclick = async () => {
  ...
  try {
    const body = await call("/api/factory/auth-app", { method: "POST", headers: allHeaders(), body: JSON.stringify({ slug, title, private: $("factoryPrivate").checked, connectBuilds: $("factoryBuilds").checked }) });
    show({ status: 201, ...body });
...
$("builderFiles").addEventListener("change", () => {
  const files = [...$("builderFiles").files];
  $("builderFileStatus").textContent = `${files.length} file(s), ${(files.reduce((sum, file) => sum + file.size, 0) / 1048576).toFixed(1)} MB total`;
});
